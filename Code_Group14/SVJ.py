...
def cf_SVJ(u, T, eta, rho, kappa, theta, Lambda, mu_J, sigma_J, V_t):
    i = 1j
    ...
    A_sv = (kappa*theta/eta**2) * ((b - d)*T - 2.0*np.log(one_minus_gexp/one_minus_g))
    B_sv = ((b - d)/eta**2) * ((1.0 - exp_neg_dT)/one_minus_gexp)

    # phi_J(u) = E[e^{iuJ}] for J ~ N(mu_J, sigma_J^2)
    phi_J = np.exp(i*u*mu_J - 0.5*sigma_J*sigma_J*u*u)

    # kappa_J = E[e^J - 1]
    kappa_J = np.exp(mu_J + 0.5*sigma_J*sigma_J) - 1.0

    # Jump exponent (compensated)
    A_jump = Lambda * T * (phi_J - 1.0 - i*u*kappa_J)

    return np.exp(A_sv + B_sv*V_t + A_jump)
...
def Lewis_call_option_pricing_sinh_tan(
    S, K, T, r, q,
    eta, rho, kappa, theta, Lambda, mu_J, sigma_J,
    V_t,
...
def loss(x, q, S, df):
    ...
    kappa, theta, eta, rho, v0, Lambda, mu_J, sigma_J = x
    ...
    errs = []
    for K, T, r0, iv_mkt in zip(Ks, Ts, rs, market_IV):

        model_price = Lewis_call_option_pricing_sinh_tan(
            S, K, T, r0, q,
            eta=eta, rho=rho, kappa=kappa, theta=theta,
            Lambda=Lambda, mu_J=mu_J, sigma_J=sigma_J,
            V_t=V_t,
            N=4096, alpha=1.2, L=24.0
        )

        iv_model = implied_vol_call(model_price, S, K, T, r0, q)
